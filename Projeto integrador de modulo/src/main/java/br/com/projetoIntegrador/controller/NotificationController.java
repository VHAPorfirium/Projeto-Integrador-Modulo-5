...
public class NotificationController {
    ...
    @Operation(summary = "Envia notificações",
               description = "Recebe uma requisição de notificação, processa o envio através do Firebase e retorna a contagem de notificações enviadas com sucesso.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Notificações enviadas com sucesso",
                         content = @Content(mediaType = "text/plain",
                                 schema = @Schema(type = "string", example = "Notificações enviadas: 5"))),
    ...
    public ResponseEntity<String> send(
            @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "Dados da requisição de notificação", required = true,
